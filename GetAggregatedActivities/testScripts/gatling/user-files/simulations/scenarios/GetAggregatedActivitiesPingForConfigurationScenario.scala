...
object GetAggregatedActivitiesPingForConfigurationScenario {
  ...
  val request = exec(
        http("GetAggregatedActivitiesPingForConfiguration")
          .post("")
          .headers(headers)
          .body(RawFileBody("GetActivitiesPingForConfiguration.xml"))
          .check(status.is(200))
          .check(substring("Applikation"))
          .check(substring("GetAggregatedActivities"))
          .check(xpath("soap:Envelope", List("soap" -> "http://schemas.xmlsoap.org/soap/envelope/")).exists)
          .check(regex("GetAggregatedActivities").exists)
      )
  }
  ...
  <soap:Body>
    <PingForConfigurationResponse
      ...
      <configuration>
        ...
        <value>GetAggregatedActivities</value>
      </configuration>
    </PingForConfigurationResponse>
  </soap:Body>
</soap:Envelope>
